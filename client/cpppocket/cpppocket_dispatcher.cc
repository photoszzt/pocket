...
CPPPocketDispatcher::CPPPocketDispatcher() {}

CPPPocketDispatcher::~CPPPocketDispatcher() {}

int CPPPocketDispatcher::Initialize(string address, int port)
{
  return this->crail_.Initialize(address, port);
}

int CPPPocketDispatcher::MakeDir(string name)
{
...
int CPPPocketDispatcher::Lookup(string name)
{
...
int CPPPocketDispatcher::Enumerate(string name)
{
  unique_ptr<CrailNode> crail_node = crail_.Lookup(name);
  if (!crail_node)
  {
...
std::vector<string> CPPPocketDispatcher::EnumerateWithReturn(string name)
{
  unique_ptr<CrailNode> crail_node = crail_.Lookup(name);
  if (!crail_node)
  {
...
int CPPPocketDispatcher::PutFile(string local_file, string dst_file,
                                 bool enumerable)
{
...
int CPPPocketDispatcher::GetFile(string src_file, string local_file)
{
...
int CPPPocketDispatcher::DeleteDir(string directory)
{
  return crail_.Remove(directory, true);
}

int CPPPocketDispatcher::DeleteFile(string file)
{
  return crail_.Remove(file, false);
}

int CPPPocketDispatcher::CountFiles(string directory)
{
  int op = 5;
  return crail_.Ioctl((unsigned char)op, directory);
}

int CPPPocketDispatcher::PutBuffer(string input_data, string dst_file,
                                   bool enumerable)
{
...
int CPPPocketDispatcher::GetBuffer(char data[], int len, string src_file)
{
  unique_ptr<CrailNode> crail_node = crail_.Lookup(src_file);
  if (!crail_node)
  {
...
string CPPPocketDispatcher::GetBufferBytes(string src_file)
{
  unique_ptr<CrailNode> crail_node = crail_.Lookup(src_file);
  if (!crail_node)
  {
