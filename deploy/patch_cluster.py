...
def prep_i3instances(private_subnetid, node_groupid):
    ...
    for i3_instance_id in i3_instance_ids:
        ...
        attach_eni_command = "aws ec2 attach-network-interface --network-interface-id " + \
            i3_eni + " --instance-id " + i3_instance_id + " --device-index 1"
...
def prep_h1instances(private_subnetid, node_groupid):
    ...
    for h1_instance_id in h1_instance_ids:
        ...
        attach_eni_command = "aws ec2 attach-network-interface --network-interface-id " + \
            h1_eni + " --instance-id " + h1_instance_id + " --device-index 1"
        print(attach_eni_command)
        sp.call(attach_eni_command, shell=True)
...
def prep_i2instances(private_subnetid, node_groupid):
    ...
    for i2_instance_id in i2_instance_ids:
        ...
        attach_eni_command = "aws ec2 attach-network-interface --network-interface-id " + \
            i2_eni + " --instance-id " + i2_instance_id + " --device-index 1"
        print(attach_eni_command)
        sp.call(attach_eni_command, shell=True)


def add_namenode_eni(vpcid):
    # get subnet id for private subnet
    cmd = ("aws ec2 describe-subnets --filters Name=tag:Name,Values='pocket-kube-public*' "
           f" Name=vpc-id,Values=\"{vpcid}\" --query \"Subnets[*].SubnetId\"")
    exitcode, out, err = get_exitcode_stdout_stderr(cmd)
    print(out)
    pattern = r'"([A-Za-z0-9_\./\\-]*)"'
    ...
    cmd = ("aws ec2 describe-security-groups --filters Name=group-name,Values='*node*' "
           f" Name=vpc-id,Values=\"{vpcid}\" --query \"SecurityGroups[*].GroupId\"")
    exitcode, out, err = get_exitcode_stdout_stderr(cmd)
    pattern = r'"([A-Za-z0-9_\./\\-]*)"'
    ...
    attach_eni_command = "aws ec2 attach-network-interface --network-interface-id " + \
        namenode_eni + " --instance-id " + namenode_instance_id + " --device-index 1"
    print(attach_eni_command)
    sp.call(attach_eni_command, shell=True)
...
def add_lambda_security_group_ingress_rule(vpcid):
    # get group id for pocket-kubernetes-lax security group
    cmd = ("aws ec2 describe-security-groups --filters Name=group-name,Values='*pocket-kube-relax-*'"
           f" Name=vpc-id,Values=\"{vpcid}\" --query \"SecurityGroups[*].GroupId\"")
    exitcode, out, err = get_exitcode_stdout_stderr(cmd)
    pattern = r'"([A-Za-z0-9_\./\\-]*)"'
    ...
    modify_security_group_command = 'aws ec2 authorize-security-group-ingress --group-id ' + \
        node_groupid + ' --protocol all --source-group ' + pocket_lax_groupid
...
def main():
    ...
    add_lambda_security_group_ingress_rule(args.vpcid)
    private_subnet_id, node_groupid = add_namenode_eni(args.vpcid)
